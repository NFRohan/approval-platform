...
function ApproversStep() {
  ...
  async function handleNext() {
    ...
    await supabase.from("approval_rules").delete().eq("form_template_id", templateId);
    ...
    const { error } = await supabase.from("approval_rules").insert(rows);
    if (error) {
    ...
    if (fieldTypes.has("money")) {
      await supabase.from("approval_slabs").delete().eq("form_template_id", templateId);
      ...
      if (slabRows.length > 0) {
        const { error: slabErr } = await supabase.from("approval_slabs").insert(slabRows);
        if (slabErr) {
